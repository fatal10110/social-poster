...
class SP_Mail_Logs_List_Table extends WP_List_Table_SP_Ex 
{
    var $social = array();
    ...
    function __construct()
    {
        parent::__construct( array(
                    'singular'  => __('Mail Log','sp_text_domain'),     //singular name of the listed records
                    'plural'    => __('Mail Logs','sp_text_domain'),    //plural name of the listed records
    ...
    function get_columns(){
          $columns = array(
            ...
            'svc' => __('Service','sp_text_domain'), 
            'account' => __('Account', 'sp_text_domain'), 
            'group' => __('Group', 'sp_text_domain'), 
    ...
    function prepare_items() {
        ...
        $sql = ' FROM `' . $wpdb->base_prefix . 'sp_logs` AS `t1` 
                    INNER JOIN `' . $wpdb->base_prefix . 'sp_mail_groups` AS `t2` ON `t1`.`acc` = `t2`.`id`
                    INNER JOIN `' . $wpdb->base_prefix . 'sp_mail_services` AS `t3` ON `t2`.`service` = `t3`.`id` '; 

        $where = ' WHERE `t1`.`type` = "1" ';
        ...
        $sql = 'SELECT  `t2`.`login` AS `account`,`t3`.`name` AS `svc`,`t2`.`blog`
                ,`t1`.`id`,`t1`.`user`,`t1`.`post` AS `pid`,`t1`.`status` AS `status`
                ,`t1`.`date`,`t1`.`acc`,`t2`.`name` AS `group` ' . $sql . $order . $limit;
    ...
    function column_default( $item, $column_name ) 
    {
   	...
   	function get_sortable_columns() {
        ...
        $not = array('blog','user','cb','date','pid','status');
    ...
    function column_status($item)
    {
        $status = array(
            ...
            '<div style="color: green;">' . __('Sent','sp_text_domain') . '</div>',
